 ...
 * @param mixed   $este  Object $this from our framework to make the callback work - UOC 20150129
 */

  function __construct($callbackHandler, $data_connector = '',$este = '') {

    if (!is_array($callbackHandler)) {
      $this->callbackHandler['launch'] = $callbackHandler;
      $this->callbackHandler['este'] = $este;// UOC victor - 20150129 - we need this to make the callback work with our controller.
    } else {
 ...
 */
  private function doCallback() {
    ...
    if (isset($this->callbackHandler[$callback])) {       
      // victor 20150129 - added este = $this , to integrate the callback with our framework
      if(!empty($this->callbackHandler['este'])){
        $este = $this->callbackHandler['este'];
        $launch = $this->callbackHandler[$callback];
        $este->$launch($this);
      }else{
        $launch = $this->callbackHandler[$callback];
        $result = call_user_func($this->callbackHandler[$este->$callback], $this);
      }
...
### Set debug mode
#
    $this->debugMode = true;//sset($_POST['custom_debug']) && (strtolower($_POST['custom_debug']) == 'true');
#
### Get the consumer
#
